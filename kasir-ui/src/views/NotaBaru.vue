<template>
  <div class="flex flex-col h-screen bg-gray-100 overflow-hidden font-sans text-black relative">
    ...
    <div v-if="form.tipe === 'SERVICE'" class="flex-1 overflow-auto p-4 md:p-8 bg-gray-50">
      <div class="max-w-5xl mx-auto space-y-8 pb-32">
        <div class="bg-white rounded-2xl shadow-sm border p-6">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="space-y-1">
              ...
              <input v-model="form.customerNama" placeholder="Nama Pelanggan" class="w-full p-3 bg-gray-50 border rounded-xl outline-none font-bold" />
            </div>
            <div class="space-y-1">
        ...
        <div class="space-y-6">
          <div v-for="(row, index) in rows" :key="index">
            <div v-if="!row.isChild" class="bg-white rounded-2xl shadow-md border overflow-hidden">
              <div class="bg-orange-600 p-4 px-6 flex justify-between items-center text-white">
                <h3 class="font-bold uppercase text-sm">Unit Service #{{ getRowNumber(index) }}</h3>
              ...
              <div class="p-6 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <input v-model="row.unitName" placeholder="Nama Unit / Model" class="p-3 bg-gray-50 border rounded-xl font-bold outline-none" />
                  <input v-model="row.kerusakan" placeholder="Keluhan / Kerusakan" class="p-3 bg-gray-50 border rounded-xl outline-none" />
                </div>
                <div class="bg-gray-50 p-4 rounded-xl border space-y-3">
                   <div v-for="(subRow, subIndex) in getRelatedRows(index)" :key="subIndex" class="flex gap-3 items-end">
                      <input v-model="subRow.solusi" placeholder="Tindakan..." class="flex-1 p-2 border rounded-lg text-sm outline-none" />
                      <input :value="formatNumber(subRow.harga)" @input="onInputMoney($event, 'row_harga', {index: rows.indexOf(subRow)})" type="text" placeholder="Harga" class="w-32 p-2 border rounded-lg text-sm text-right font-bold outline-none" />
                   </div>
                   <button @click="addSolution(index)" class="text-[10px] text-orange-600 font-bold uppercase">+ Tambah Tindakan</button>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="fixed bottom-0 left-0 right-0 p-6 bg-white border-t z-50 flex justify-center">
         <button @click="saveNota" class="bg-orange-600 text-white px-20 py-4 rounded-2xl font-black uppercase shadow-xl hover:bg-orange-700 transition">Simpan Transaksi Service</button>
      </div>
    </div>
    ...
    <div v-else class="flex-1 flex overflow-hidden">
      ...
      <div class="w-full md:w-[400px] flex flex-col bg-white border-r shadow-2xl z-30 overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
          <input v-model="form.customerNama" placeholder="Nama Pelanggan *" class="w-full border-2 p-3 rounded-xl text-sm font-bold outline-none focus:border-blue-500 shadow-sm mb-2" />
          
          <div v-for="(row, index) in rows" :key="index" v-show="row.namaBarang && row.itemId" class="flex justify-between items-center p-3 bg-white rounded-xl border shadow-sm group">
            <div class="flex-1 min-w-0">
              <p class="font-bold text-xs text-slate-800 uppercase truncate">{{ row.namaBarang }}</p>
        ...
        <div class="p-4 bg-white border-t border-gray-200 space-y-3">
          <div class="grid grid-cols-2 gap-3">
            <div class="bg-gray-50 p-2 rounded-lg border">
              <label class="text-[9px] font-black text-gray-400 uppercase block mb-1">Uang Muka (DP)</label>
            ...
            <div class="bg-blue-50 p-2 rounded-lg border border-blue-100">
              <label class="text-[9px] font-black text-blue-400 uppercase block mb-1">Cash / Bayar</label>
          ...
          <div class="flex justify-between p-3 bg-slate-100 rounded-xl">
            <div class="text-left border-r pr-4 border-gray-300">
               <p class="text-[9px] font-bold text-gray-500 uppercase">Sisa</p>
               <p class="font-mono font-black text-red-600">{{ formatNumber(sisaBayar) }}</p>
            </div>
            <div class="text-right">
               <p class="text-[9px] font-bold text-gray-500 uppercase">Kembali</p>
               <p class="font-mono font-black text-green-600">{{ formatNumber(kembalian) }}</p>
            </div>
          </div>

          <button @click="saveNota" class="w-full bg-blue-600 hover:bg-blue-700 text-white py-3.5 rounded-xl font-bold uppercase text-xs tracking-widest shadow-lg transition transform active:scale-95 flex items-center justify-center gap-2">
      ...
      <div class="flex-1 bg-gray-100 flex flex-col overflow-hidden">
        <div class="p-4 bg-white border-b shadow-sm">
          <div class="relative max-w-2xl mx-auto">
            ...
            <input v-model="search" @keyup.enter="handleBarcodeEnter" ref="barcodeField" placeholder="Ketik nama barang atau Scan Barcode disini..." class="w-full pl-12 p-4 rounded-2xl bg-gray-100 outline-none focus:bg-white focus:ring-4 focus:ring-blue-100 transition text-sm font-bold shadow-inner" />
          </div>
        </div>

        <div class="flex-1 overflow-y-auto p-6">
          <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-5">
            <div v-for="item in filteredItems" :key="item.id" @click="onProductClick(item)" class="bg-white p-3 rounded-[2rem] border-2 border-transparent hover:border-blue-500 cursor-pointer shadow-sm active:scale-95 transition-all text-center group">
              <div class="w-full aspect-square bg-gray-50 rounded-[1.5rem] mb-2 flex items-center justify-center overflow-hidden border">
              ...
              <p class="text-blue-600 font-bold text-[11px] font-mono mt-1">{{ formatNumber(item.harga) }}</p>
            </div>
          </div>
        </div>
      </div>
...
const handleBarcodeEnter = () => {
  ...
  if(matched) {
    ...
    search.value = ""; // Clear input after scan
  }
};
...
const addSolution = (index) => {
  rows.value.splice(index + 1, 0, { solusi: "", harga: 0, qty: 1, isChild: true });
};

const removeRow = (index) => {
  rows.value.splice(index, 1);
  if(rows.value.length === 0) rows.value = [{ unitName: "", kerusakan: "", solusi: "", namaBarang: "", qty: 1, harga: 0, itemId: null, isChild: false }];
};

const getRelatedRows = (mainIdx) => {
  const res = [];
  for(let i = mainIdx; i < rows.value.length; i++) {
    if(i === mainIdx || rows.value[i].isChild) res.push(rows.value[i]);
    else break;
  }
  return res;
};
...
const saveNota = async () => {
  ...
  const payload = {
    ...
    items: rows.value.filter(r => r.namaBarang || r.solusi).map(r => ({
      ...
      hargaSatuan: r.harga,
      jumlah: r.qty,
      catatan: form.value.tipe === 'SERVICE' ? `Service Unit` : 'Retail'
    }))
  };
