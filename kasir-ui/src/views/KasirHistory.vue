      ...
      <div class="flex flex-col md:flex-row justify-between gap-4 items-center">
        ...
        <div class="flex bg-gray-100 p-1 rounded-lg">
          <button 
            ...
            class="px-6 py-2 rounded-md font-bold transition-all duration-200 flex items-center gap-2">
        ...
        <div class="flex gap-2">
          <select v-model="selectedMonth" class="border p-2 rounded bg-gray-50 focus:ring-2 focus:ring-blue-500 outline-none">
            <option v-for="(bulan, index) in namaBulan" :key="index" :value="index + 1">
              {{ bulan }}
            </option>
          </select>
          
          <select v-model="selectedYear" class="border p-2 rounded bg-gray-50 focus:ring-2 focus:ring-blue-500 outline-none">
...
// === STATE FILTER ===
const activeTab = ref('SERVICE'); // Default Tab: SERVICE
...
// Generate List Tahun (Misal: 5 tahun ke belakang s/d 1 tahun ke depan)
...
const filteredNota = computed(() => {
  return listNota.value
    ...
    // 3. Sorting DESC (Terbaru di atas)
    .sort((a, b) => b.id - a.id);
});
